...
SELECT A.*
FROM "ADHOC_DB"."PUBLIC"."FCT_USGS_CATALOG" A
JOIN (SELECT MAX(MAG) AS MAX_MAG FROM "ADHOC_DB"."PUBLIC"."FCT_USGS_CATALOG" WHERE TYPE = 'earthquake') B
ON A.MAG = MAX_MAG
...
FROM "ADHOC_DB"."PUBLIC"."FCT_USGS_CATALOG"
...
SELECT A.*
FROM "ADHOC_DB"."PUBLIC"."FCT_USGS_CATALOG" A
JOIN (SELECT SIG FROM "ADHOC_DB"."PUBLIC"."FCT_USGS_CATALOG" WHERE TSUNAMI = 1 ORDER BY 1 DESC LIMIT 10) B
ON A.SIG = B.SIG
WHERE TSUNAMI = 1;
